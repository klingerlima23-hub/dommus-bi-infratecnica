"""
Sincroniza f_historico_etapa_processo (origem dommus_infratecnica.tb_controle_sla
-> destino dw_infratecnica.f_historico_etapa_processo).

Padrao: TRUNCATE + FULL LOAD com keyset pagination em e.id (alias 'id').

Diferenca para f_evolucao_processo:
- f_evolucao_processo le da view view_controle_sla_mais_recentes (apenas registros
  mais recentes por processo/etapa).
- f_historico_etapa_processo le direto de tb_controle_sla (HISTORICO COMPLETO de
  transicoes), incluindo os nomes das etapas anterior e atual.
...
# ---------- parametros ----------
TABELA_DESTINO = "f_historico_etapa_processo"
PK_QUALIFIED   = "e.id"
PK_ALIAS       = "id"
CAMINHO_SQL    = caminho_sql("consulta_f_historico_etapa_processo")
