"""
Sincroniza d_usuario (origem bi_dommus -> destino dw_infratecnica.d_usuario).

Padrao: TRUNCATE + FULL LOAD com keyset pagination em
        CAST(RI.json_externo AS UNSIGNED) (alias id_usuario no resultado).

O id_usuario no dw vem do campo json_externo da tb_referencia_integracao
(convertido para UNSIGNED), entao a paginacao eh feita na mesma expressao
para que o last_id retornado pela pagina seja coerente com o proximo offset.
...
# ---------- parametros ----------
TABELA_DESTINO = "d_usuario"
PK_QUALIFIED   = "CAST(RI.json_externo AS UNSIGNED)"
PK_ALIAS       = "id_usuario"
CAMINHO_SQL    = caminho_sql("consulta_d_usuario")

# Filtro de data esta comentado no SQL. Mantemos os valores para caso
# o usuario descomente a linha "-- AND US.CADASTRADO_EM between ..."
...
def main():
    ...
    # Aplica format somente se houver placeholders ativos
    if "{data_inicio}" in sql_raw or "{data_fim}" in sql_raw:
